function [G, W, MSPE, min_MSPE, lambda_min_MSPE, it_updates, Matlab_ans, FitInfo] = SGD_lasso(Y,X,b)
...
for g_index = 1:quant_lambdas
    ...
    g = 1/b*( -1/N*x(rand_obs,:)'*y(rand_obs) + 1/N*x(rand_obs,:)'*x(rand_obs,:)*w + 1/N*lambda*sign(w) );
    w = w - alpha*( g );% + 1/N*S_lambda(lambda,w) ); %        
    ...
    it_updates = it_updates + 1; % other option, but faster decreasing: alpha = alpha/sqrt(k);
    end % of "while"  
